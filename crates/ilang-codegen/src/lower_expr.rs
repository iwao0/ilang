    ...
    for (i, arm) in arms.iter().enumerate() {
        ...
        let eq_v = match (&arm.pattern.kind, st) {
            ...
            (
                ...
                let true_v = b.ins().iconst(I8, 1);
                let lo_ok = match low {
                    Some(lo) => {
                        let v = b.ins().iconst(cl, *lo);
                        b.ins().icmp(lo_cc, sv, v)
                    }
                    None => true_v,
                };
                let hi_ok = match high {
                    Some(hi) => {
                        let v = b.ins().iconst(cl, *hi);
                        b.ins().icmp(
...
fn lower_closure_construct(
    ...
    for (i, (cap_name, _cap_ty)) in captures.iter().enumerate() {
        ...
        // from that wrapper's already-captured `this` env slot. The
        // hoist pass only injects this entry when the closure body
        ...
        let resolved: Option<(cranelift::prelude::Value, JitTy)> =
            if is_this_capture {
                if let Some((var, class_id)) = lc.this {
                    Some((b.use_var(var), JitTy::Object(class_id)))
        ...
        // Doesn't apply to the synthetic `this` capture.
        let (v, jty) = if let Some(tv) = resolved {
            tv
        } else if !is_this_capture {
            if let Some(&slot) = lc.global_let_slots.get(cap_name) {
