...
fn appkit_dir() -> PathBuf {
    repo_root().join("bindings/cocoa/appkit")
}

fn test_dir() -> PathBuf {
    appkit_dir().join("test")
}
...
fn parse_binding_selectors() -> BTreeMap<String, Vec<SelEntry>> {
    ...
    let Ok(entries) = fs::read_dir(appkit_dir()) else {
        return classes;
    };
    for entry in entries.flatten() {
        let p = entry.path();
...
#[test]
fn run_appkit_fixtures() {
    if !cfg!(target_os = "macos") {
        eprintln!("skipping: appkit tests only run on macOS");
        return;
    }
