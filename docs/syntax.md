...
関数は値として変数に代入したり、引数や戻り値として渡せます。匿名関数の本体は外側のローカル変数を **値でキャプチャ** できます (interpreter のみ — JIT は現状キャプチャありクロージャ非対応)。
...
- **interpreter のみ** — JIT は現状キャプチャ無しの匿名関数のみ対応 (キャプチャを参照したクロージャを JIT で実行しようとすると "unknown variable" エラー)。クロージャ struct + env_ptr の JIT 実装は別フェーズ
- 匿名関数 (キャプチャ無し) の JIT は従来どおりトップレベルへホイスト (`func_addr` + `call_indirect`)
