...
- **値渡し (`by_value` フラグ)**: `@extern(by_value) fn sum_point(p: point): i64` または `@extern("libfoo", by_value) fn ...` のように宣言すると、`@repr(C)` クラスの引数が **値渡し** で C 関数に渡される。実装は AArch64 AAPCS64 / x86_64 SysV 共通の "integer-only ≤ 16 B composite" ルールに従い、struct のバイトを 1〜2 個の i64 chunk に分解して呼び出し signature に展開する
   - フィールド制約: 整数 / bool のみ (float/double / nested struct / array / string は HFA / aggregate ABI 別実装が必要なので登録時に拒否)
   - **サイズ別の引数渡し**:
   ...
   - **戻り値の値渡し** にも対応 (≤ 16 B integer struct のみ): C 側が GPR ペアに詰めて返した chunk を JIT 側で受け取り、新規 instance を alloc して書き戻す。caller は rc=1 の通常 Object を見る。> 16 B return は sret ABI が必要なため未対応
