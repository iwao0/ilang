...
- **値渡し (`by_value` フラグ)**: `@extern(by_value) fn sum_point(p: point): i64` または `@extern("libfoo", by_value) fn ...` のように宣言すると、`@repr(C)` クラスの引数が **値渡し** で C 関数に渡される。実装は AArch64 AAPCS64 / x86_64 SysV 共通の "integer-only ≤ 16 B composite" ルールに従い、struct のバイトを 1〜2 個の i64 chunk に分解して呼び出し signature に展開する
...
- **`slice_return` フラグ**: `@extern(slice_return) fn foo(): T[]` のように宣言すると、C 側は `{ T* ptr; size_t len; }` の 16 B struct を返している前提で、JIT が境界で `(ptr, len)` を 2 GPR で受け取り → 新規 ilang `T[]` を alloc → `len * sizeof(T)` バイトを memcpy する。ユーザコードからは普通の `T[]` に見えるので raw pointer は露出しない (Rust の `unsafe { slice::from_raw_parts(ptr, len) }` を境界の 1 行に閉じ込める考え方と同じ)
   ...
   - **失敗時 NULL** に対応: 戻り型を `T[]?` にすると `ptr == NULL` を `None` にマッピング (例: `@extern(slice_return) fn maybe_get(): u8[]?` → C 側が `(NULL, 0)` を返したら ilang 側は None)。`T[]` (non-Optional) の場合は NULL を空配列に変換するので segfault しない
