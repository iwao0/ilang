...
- **値渡し (`by_value` フラグ)**: `@extern(by_value) fn sum_point(p: point): i64` または `@extern("libfoo", by_value) fn ...` のように宣言すると、`@repr(C)` クラスの引数が **値渡し** で C 関数に渡される。実装は AArch64 AAPCS64 / x86_64 SysV 共通の "integer-only ≤ 16 B composite" ルールに従い、struct のバイトを 1〜2 個の i64 chunk に分解して呼び出し signature に展開する
   - 制約: ≤ **16 B** の `@repr(C)` クラスのみ。整数 / bool / 数値配列フィールドのみ (float/double フィールド・nested 文字列・>16 B は未対応 — 実 ABI では floating-point register 経由 / sret 経由になるため別実装が必要)
   - **戻り値の値渡し** にも対応 (≤ 16 B integer struct のみ): C 側が GPR ペアに詰めて返した chunk を JIT 側で受け取り、新規 instance を alloc して書き戻す。caller は rc=1 の通常 Object を見る
