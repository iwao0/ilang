...
- **`struct Name { fields }`** — C 互換構造体 (旧 `@repr(C) class` の置き換え)
...
- **`@lib("name", "fallback", ...)`** — dlopen するライブラリ名。複数指定すると先頭から順に試し、最初に開けたものを使う (soname 違いの吸収)。`@lib` を省略すると **host 登録形** (`JITBuilder::symbol(...)` でホストが事前登録した関数アドレスを使う、`math` / `os` / `test` 標準ライブラリの実装方式)
