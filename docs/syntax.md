...
- **C コールバック (関数ポインタ引数)**: extern fn のパラメータに `fn(...)` 型を書ける (例: `compar: fn(i64, i64): i32`)。引数の中身/戻り値は **数値プリミティブのみ** (string / object / Optional 不可)。呼び出し側は **キャプチャを持たないトップレベル fn 名を直接渡す** 必要がある — `let f = my_fn; ext(f)` のように let 経由で渡すと拒否される (closure box には C ABI に乗らない env_ptr スロットが付くため)
- **数値配列をバッファとして渡す**: `i64[]` / `u8[]` / `f64[]` 等の数値要素配列は **`void *` (生のデータポインタ)** として C に渡る。要素型は数値プリミティブのみ (`bool` / 文字列 / Object / 配列の配列は不可)。長さ情報は C 側に渡らないので `n: u64` のような明示引数で渡す慣例。`memcpy(dst, src, n)` のような書き込み API も動作 — ARC が呼び出し中の解放を防ぐので C 側がデータを上書きしても安全
- **複数ライブラリ名のフォールバック**: `@extern("libssl.so.3", "libssl.so.1.1") fn SSL_write(...)` のように Str 引数を複数並べると、最初の名前で失敗したら次を順に試す。ディストロ / OS / バージョン違いの soname を吸収する用途。`os.libLoaded(name)` の問い合わせは **常に最初 (canonical) の名前** で行う — どの候補がロードされたかに関わらず
- **オプショナルなライブラリ (`optional` フラグ)**: `@extern("libname", optional) fn ...` と書くと、ライブラリが見つからなくても **JIT 構築は失敗しない**。代わりに該当 fn は「呼ばれたら abort する」スタブにバインドされる。プログラム側で `os.libLoaded("libname"): bool` を使って事前にガードする慣例:
   ```rust
   use os
   @extern("libssl.so.3", optional) fn SSL_write(...)
   ...
   ```
   `optional` はライブラリ自体が無い場合だけでなく、ライブラリは見つかったが個別シンボルが解決できない場合にも適用される。ガードなしで呼ぶと `runtime panic: called an @extern(..., optional) fn from a missing library` で停止
- **out-pointer (sqlite3_open 形式) のサポート**: C で `int sqlite3_open(const char *, sqlite3 **)` のように「呼び出し側がスロットを用意して、関数がそこに書き込む」イディオムは、**1要素の `i64[]` をスロット**として使い、書き込まれたポインタを **`as Foo` で opaque-extern クラスにキャスト** する。例:
   ```rust
   @extern("c") class Buf {}
   @extern("c") fn posix_memalign(memptr: i64[], align: u64, size: u64): i32

   let slot: i64[] = [0]
   if posix_memalign(slot, 64, 1024) == 0 {
       let buf = slot[0] as Buf       // i64 → opaque クラスへの cast
   }
   ```
   キャストは **`deinit` を持たない opaque-extern クラス** にのみ許される (raw ポインタ・モード)。`deinit` 持ちの ARC ボックスを raw ptr から作るのは安全に行えないため意図的に拒否される。逆方向 (`Foo as i64`) も同じ制限で許可
...
- C 側が **ヒープに確保した文字列を返す** 場合は `@extern("lib", ownedReturn)` を付けると、JIT がコピー後に `libc::free(ptr)` を呼んでメモリを解放する (`strdup` 等)。`ownedReturn` は `string` 戻り値の fn にしか付けられない (型チェック)
- ライブラリが **専用の解放関数** を持つ場合 (SQLite の `sqlite3_free`、libxml2 の `xmlFree`、OpenSSL の `OPENSSL_free` 等) は `freeWith.<fn_name>` で指定: `@extern("sqlite3", ownedReturn, freeWith.sqlite3_free) fn sqlite3_mprintf(fmt: string): string`。指定する fn は同プログラム中に `@extern` で宣言されていて、引数 1 個 (`i64` または `@extern` クラス) で戻り値は無くてよい。`libc::free` でない領域を `libc::free` で開放するとヒープ崩壊するので、ライブラリのドキュメントに従って必ず指定する
- **C union (`@repr(C, union) class`)**: 全フィールドが offset 0 に重ねて配置される。クラス全体のサイズは `max(field_sizes)`、align は `max(field_aligns)`。一つのフィールドに書くと他フィールドの bytes も上書きされる (C union と同じ semantics)
   - 用途: C interop で `union sigval` / `siginfo_t` 等を扱う、または整数 ↔ float の bit pattern 変換 (type punning)
   - 制約: フィールドは **数値プリミティブ / bool / 固定長数値配列** のみ。heap 型 (string / Object / array) は禁止 (storage を共有すると ARC が壊れる)。bitfield と FAM は禁止、`packed` との併用も禁止
   - **JIT only** (インタプリタは shared-storage 表現を持たない)
- **C 構造体 (`@repr(C) class`)** / **`@repr(C, packed)`**: C のシステムコールが期待する `struct stat`、`struct timespec` のような構造体型を ilang から扱える。`packed` 修飾子で `__attribute__((packed))` 相当のアラインメント無し配置 (各フィールドが `offset = sum of prior sizes` に置かれ、構造体全体の align も 1) が選べる。ネットワーク/ファイルフォーマットのヘッダ (`{ magic: u8; length: u32; ... }`) など、padding を入れたくないケース向け
   ...
   let ts = new timespec()              // 0 初期化されたインスタンス
   clock_gettime(0 as i32, ts)          // C 側が ts のメモリに書き込む
   console.log(ts.tv_sec)               // C が書いた値が読める
   ```

   - メソッド / `init` / 継承 / 型パラメータ / プロパティは禁止 (フィールドのみ)
   - 各フィールドは natural alignment (i64 なら 8B 境界、i32 なら 4B 境界、bool なら 1B) で配置 — C struct のレイアウトと一致
   - `new ClassName()` で **0 初期化** (alloc_object のゼロクリア領域がそのまま使える)
   - native extern の引数/戻り値型として `Type::Object(name)` を受け付け、ユーザポインタ (= フィールド領域の先頭) として C に渡る (ARC ヘッダは負オフセットに置かれて C からは見えない)
   - フィールドの型は **数値プリミティブ・bool・`string`・他の `@repr(C)` クラス・固定長数値配列** が許される
   - **`string` フィールド**: 8 バイトの heap ポインタ (`StringRc *`) をスロットに保持する。`new` で **interned 空文字列** に初期化されるので未代入で読んでも安全。代入で旧値が ARC release され、クラス drop で残っている文字列も release される。**注意**: 物理レイアウトは `char *` ではなく `StringRc *` なので、C 関数にこの構造体ポインタを直接渡しても C 側で `char *` として読むと正しくない。strict な C ABI で `char *` メンバを必要とする場合は (a) 関数引数として `string` を別途渡す (既存の str→c_str マーシャリング)、または (b) `u8[N]` の固定長 buffer を使う
   - **固定長数値配列フィールド** (`u8[8]`, `i64[3]`, `f64[2]` 等): バイト列がインライン埋め込みされる (heap header なし、ARC 対象外)。要素単位アクセス `s.arr[i]` で読み書き可。長さは型レベルで分かるので **bounds check は実行時に有効**。`outer.arr` を単独でアクセスする (full-array read) のは未対応 — 必ず添字を伴う
   - **nested struct**: `@repr(C)` クラスを別の `@repr(C)` クラスのフィールド型として使うと、内側の struct のバイト列が外側のメモリに **インラインで埋め込まれる** (ポインタ ではなく)。chain access (`outer.inner.x`) で読み書き可能。`struct stat` の `st_atimespec` のような POSIX 型がそのまま表現できる
   - **aggregate literal**: `point { x: 1 as i32, y: 2 as i32 }` のように波括弧でフィールドを並べてリテラル構築できる。`new ClassName()` + 連続 field 代入の糖衣。nested 構造体リテラルもネスト可: `rect { origin: point { x: 1, y: 2 }, width: 100, height: 50 }`
   - 内部宣言順序: 内側 class を後に宣言してもよい — JIT が `@repr(C)` クラス間の埋め込み依存を **トポロジカルソート** してから layout を確定する。循環埋め込みは無限サイズになるためエラー
   - **C99 flexible array member**: 最終フィールドに `T[]` (固定長なし) を書くと flexible array member になる。`new ClassName(n)` で `sizeof(固定領域) + n * sizeof(T)` バイトを確保し、`obj.data[i]` で末尾のバイト列を要素単位アクセスできる。bounds check は **省略** (C の慣習通り、ユーザがカウントを sibling field 等で管理する責任を持つ)。要素型は数値プリミティブ / bool
   - **`@bits(N)` ビットフィールド**: `@bits(3) mode: u32` のように宣言すると、フィールドが N ビット幅のビットフィールドになる。連続する同じ underlying 型のビットフィールドは **共有ストレージユニット** にパッキングされる (GCC-style: ユニット境界をまたがない、別 underlying 型 / 非ビットフィールドが入ったら新ユニット)。read = `(load >> bit_offset) & mask`、write = read-modify-write でユニット内の他ビットフィールドを保護
     - 制約: **unsigned 整数型のみ** (u8/u16/u32/u64)、`@repr(C)` クラス内のみ、N は 1..=underlying 幅
     - はみ出る値は黙ってマスク (`reserved = 255 as u32` を 4-bit 幅に書くと 0xF が入る) — C のビットフィールド代入と同じセマンティクス
- **値渡し (`byValue` フラグ)**: `@extern(byValue) fn sum_point(p: point): i64` または `@extern("libfoo", byValue) fn ...` のように宣言すると、`@repr(C)` クラスの引数が **値渡し** で C 関数に渡される。実装は AArch64 AAPCS64 / x86_64 SysV 共通の "integer-only ≤ 16 B composite" ルールに従い、struct のバイトを 1〜2 個の i64 chunk に分解して呼び出し signature に展開する
   - フィールド制約: **整数 / bool** または **HFA** (1..=4 個の同一 float 型フィールドのみ)。nested struct / array / string / 混在 (int+float) は別実装が必要なので登録時に拒否
   - **サイズ別の引数渡し**:
     - ≤ 8 B: 1 個の i64 chunk (single GPR)
     - 9..=16 B: 2 個の i64 chunk (GPR pair)
     - **> 16 B**: indirect — caller が copy を確保してポインタ渡し。target 別実装:
       - **x86_64 SysV**: Cranelift の `StructArgument(size)` で stack copy
       - **aarch64 AAPCS64**: stack slot を確保 → struct を chunk-copy → address を i64 で渡す (AArch64 backend は `StructArgument` 未サポートのため手動 emulate)
     - **HFA** (Homogeneous Floating-point Aggregate): 1..=4 個の同一 float 型フィールド (`vec3 { x: f32, y: f32, z: f32 }` 等)。各 field が独立した FP register slot (V0..V3 / XMM) に乗る。GPR chunking はせず、AbiParam(F32/F64) を field 数ぶん並べる
   - **戻り値の値渡し**:
     - ≤ 16 B integer struct: GPR ペアで返ってきた chunk を JIT 側で受け取り、新規 instance に書き戻す
     - HFA: FP register set (V0..V3 / XMM) で返ってきた要素を field 単位で書き戻す
     - **> 16 B integer struct (sret)**: caller が事前に instance を alloc してその pointer を hidden 第 1 引数として渡す (Cranelift `ArgumentPurpose::StructReturn` → AArch64 X8 / x86_64 SysV RDI)。callee がそのバッファに書き込み、JIT は同じ pointer を rc=1 の Object として返す
- **可変引数 (`variadic` フラグ)**: `@extern("c", variadic) fn snprintf(buf: u8[], n: u64, fmt: string): i32` のように宣言すると、宣言した固定 prefix の後に **任意数の追加引数** を呼び出せる。各呼び出しで Cranelift の signature が動的に構築され、追加引数は実際の JIT 型のまま C 側に流れる。
   - 固定 prefix の型は通常通りチェック (例: `fmt` は string でなければならない)
   - 追加引数は型不問だが、呼ぶ側がフォーマット指定子と一致させる責任を持つ (`%d` には i32/i64、`%f` には f64、`%s` には string など)
   - **Apple AArch64 対応**: macOS (arm64) は variadic 引数をスタック経由で渡す ABI のため、JIT がダミー引数で X0–X7 / V0–V7 レジスタを埋めて、実際の variadic 引数がスタックに溢れるように signature を組む (Linux x86_64 では System V ABI の通常動作のためパディング不要)
- **`winFastcall` フラグ**: `@extern("user32", winFastcall) fn MessageBoxA(...)` のように宣言すると signature の calling convention が `CallConv::WindowsFastcall` (Windows x64 ABI) に切り替わる
   - **Windows 上でのみ有効** — macOS / Linux では宣言した DLL の dlopen が失敗するため動作しない (preparation として追加されたフラグ。ilang 自体は Windows ターゲットでビルドされていれば呼び出せる)
   - 32-bit Windows の `stdcall` / `fastcall` は modern Cranelift がサポートしていないので未対応
- **`cstrArray` フラグ**: `@extern("c", cstrArray) fn get_things(): string[]` のように宣言すると、C 側は **NUL 終端の `char**`** (NULL pointer terminator まで続く `char*` 配列) を返す前提で、JIT がランタイムヘルパーで NULL まで走査 → 各 `char*` を ilang `string` にコピー → `string[]` を構築する
   - 戻り型は `string[]` のみ
   - 用途: `environ` のような POSIX グローバルの snapshot コピー、glib の `g_strsplit` 等の string-vector を返す API
   - 各文字列はコピーされるので C 側 buffer の lifetime と切り離される。NULL 入力 / 空配列はサイズ 0 の ilang 配列に
- **`out<T>` パラメータ**: `@extern fn parse_int(s: string, out_val: out<i32>): i32` のように宣言すると POSIX 流の "ポインタ経由で書き戻す" out 引数になる。JIT が呼び出し時に stack slot を確保 → C には `*mut T` を渡し → 呼び出し後にスロットを読む。ユーザコードからは pointer が見えず、`out<T>` の inner T が戻り値タプルに追加される
   - C 戻り値が `T1` で `out<T2>` が 1 つ → ilang 戻り値は `(T1, T2)` のタプル (`r[0]` で status, `r[1]` で値)
   - `out<T1>`/`out<T2>` 複数 + 戻り `T0` → `(T0, T1, T2, ...)` のタプル
   - `()` (Unit) 戻り + `out<T>` 1 つ → 単一値 `T` (タプル化しない)
   - inner T は数値プリミティブ / bool のみ。Rust の `&mut T` を FFI 境界の 1 行に閉じ込める考え方と同じ
- **`errnoCheck` フラグ**: `@extern(errnoCheck) fn open(path: string, flags: i32): i32?` のように宣言すると、POSIX 流の "戻り値 < 0 は失敗" 規約に従って、JIT が呼び出し後に `result < 0` を分岐し、失敗時は `None`、成功時は `Some(value)` を返す
   - 戻り型は `i32?` / `i64?` のみ (signed compare)
   - エラー原因は別途 `os.errno()` で読む (既存機構、TLS 経由で `__errno_location` / `__error` を呼んで取得済み)
   - 用途: `open` / `read` / `write` / `socket` / `accept` 等の POSIX システムコール
- **`sliceReturn` フラグ**: `@extern(sliceReturn) fn foo(): T[]` のように宣言すると、C 側は `{ T* ptr; size_t len; }` の 16 B struct を返している前提で、JIT が境界で `(ptr, len)` を 2 GPR で受け取り → 新規 ilang `T[]` を alloc → `len * sizeof(T)` バイトを memcpy する。ユーザコードからは普通の `T[]` に見えるので raw pointer は露出しない (Rust の `unsafe { slice::from_raw_parts(ptr, len) }` を境界の 1 行に閉じ込める考え方と同じ)
   - 制約: `T` は数値プリミティブ / bool のみ。bytes は ilang 側に **コピー** される (C 側の buffer は呼び出し直後に解放されてもよい)
   - 用途: C 側で確保された配列を ilang に取り込む (configuration tables、fixed snapshots、parser results など)
   - **失敗時 NULL** に対応: 戻り型を `T[]?` にすると `ptr == NULL` を `None` にマッピング (例: `@extern(sliceReturn) fn maybe_get(): u8[]?` → C 側が `(NULL, 0)` を返したら ilang 側は None)。`T[]` (non-Optional) の場合は NULL を空配列に変換するので segfault しない
- **`@extern static` グローバル変数**: `@extern("libname") static name: T` (dlsym で symbol address を解決) または `@extern static name: T` (host が `register_static_addrs` で事前登録した address を使用)。read/write は load/store として直接 address に対して行われるので、C 側の global と双方向にやり取りできる
   - 型は **数値プリミティブ / bool** のみ (string や struct は marshalling が必要なので未対応)
   - 用途: `errno` / `stdin` / `stdout` / `optarg` / `tzname` 等の C global 直接アクセス
   - JIT のみ対応 (interpreter には dlsym パスがない)
...
@extern("c", ownedReturn) fn strdup(s: string): string
...
- `os.libLoaded(name: string): bool` — 指定の `@extern("name")` ライブラリがロードに成功したかを返す。`optional` フラグ付き fn を呼ぶ前のガードに
...
値は macOS / Linux glibc で一致するもののみ収録。プラットフォームで異なる定数 (`EAGAIN`, `O_CREAT`, `O_TRUNC` 等) は意図的に含めず、必要ならドキュメント参照のうえハードコードするか、`@extern("c")` で直接呼び出すこと
