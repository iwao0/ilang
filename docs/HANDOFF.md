...
ilang から `wgpu-native` (WebGPU の C 実装) を叩く PoC。**SDL2 で独立ウィンドウを作り、その上に wgpu サーフェスを張って WGSL シェーダで三角形を描く**。「シェーダを環境非依存(WGSL 1本)で 3 OS」を狙う検証。**macOS / Windows 実機確認済み**。**Linux (X11/Wayland) はコード実装済みだが実機未検証**。サーフェスソースは `os.platform` で出し分ける(macOS=CAMetalLayer / Windows=HWND / Linux=X11 or Wayland を `info.subsystem` で分岐)。バインドは `bindings/wgpu/` (wgpu-native **v29.0.0.0** の `webgpu.h`/`wgpu.h` に固定)。
...
### OS 別 SurfaceSource (3 OS すべて実装済み — Linux は実機未検証)
...
- **Linux**: `info.subsystem` で X11 (=2) / Wayland (=6) を分岐。X11 は `WGPUSurfaceSourceXlibWindow` (sType=6)、Wayland は `WGPUSurfaceSourceWaylandSurface` (sType=7)。**Windows 実行で型チェックは通るが Linux 実機では未検証** — 動かないときはまず単色クリアから確認する。
...
- **Linux は未実機検証**(macOS/Windows は確認済み)。X11/Wayland の SurfaceSource は実装済みで Windows 実行では型チェックを通っているが、実機で動かす際はまず単色クリア → 三角形の順で確認するのが安全。
