...
ilang から `wgpu-native` (WebGPU の C 実装) を叩く PoC。**SDL2 で独立ウィンドウを作り、その上に wgpu サーフェスを張って WGSL シェーダで三角形を描く**。「シェーダを環境非依存(WGSL 1本)で 3 OS」を狙う検証。**macOS / Windows 実機確認済み**(Linux は未検証)。サーフェスソースは `os.platform` で出し分ける(macOS=CAMetalLayer / Windows=HWND)。バインドは `bindings/wgpu/` (wgpu-native **v29.0.0.0** の `webgpu.h`/`wgpu.h` に固定)。
...
### OS 別 SurfaceSource (macOS / Windows は実装済み、Linux が残り)

`examples/wgpu_triangle/main.il` は `os.platform` を見て SurfaceSource を出し分ける。共通部 (adapter/device/pipeline/draw) は OS 非依存。

- **macOS** (実装済み): `SDL_Metal_CreateView` → `SDL_Metal_GetLayer` → `WGPUSurfaceSourceMetalLayer` (sType=4)。
- **Windows** (実装済み): `SDL_GetWindowWMInfo` で HWND/HINSTANCE を取り `WGPUSurfaceSourceWindowsHWND` (sType=5) をチェイン。ウィンドウフラグは macOS だけ METAL を付け、それ以外は SHOWN のみ。
- **Linux** (未実装): 下記の X11 / Wayland SurfaceSource を同じ要領で追加すれば動くはず。
...
各 OS の SurfaceSource を `WGPUSurfaceDescriptor.nextInChain` にチェインする (struct は `bindings/wgpu/mod.il`):

- **Windows** (`info.subsystem == 1`): handle1=HWND, handle3=HINSTANCE — **実装済み**
  ```
  ...
  ```
- **Linux/X11** (`info.subsystem == 2`): handle1=Display*, handle2=Window(XID) — struct を mod.il に追加して分岐を生やす
  ```
...
- **Linux は未実機検証**(Windows は確認済み)。X11/Wayland の SurfaceSource を追加したら、まず単色クリア → 三角形の順で確認するのが安全。
