...
**既知の残り**: REPL slot は固定長配列を正しく扱えない (`xs.length` がバッファ先頭を length と誤読 — **HEAD でも同じ挙動の既存ギャップ**、stash して確認済み)。generic 経由の間接インスタンス化 (`fn f<T>(x: T?)` に fixed を渡して T が fixed に解決する形) は checker の制限をすり抜けるが、注釈で直接書く形は全て封鎖済み。`Box<Box[2]>` のような generic class field への mono は field サポートに乗るため動く見込みだが未 probe。
