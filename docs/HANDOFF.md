...
- **`docs/syntax.md` 等を整形する formatter (`ilang-lsp`) が `examples/wgpu_triangle/main.il` の WGSL バッククォート テンプレートリテラルをラウンドトリップできず、`formatter_preserves_lexability_on_corpus` テストが落ちる**。codegen とは無関係の formatter 側既存バグ (template literal の中身を rewrap/spacing が壊す)。未修正。
