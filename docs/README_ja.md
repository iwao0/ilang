...
| `sort` (20万)   | 0.01s | 0.01s | 0.02s     | 0.01s     | 0.05s   | 0.06s | 0.22s  |
...
¹ `time -p` の 10 ms 解像度より下。`s = s + expr` のシェイプを MIR で検出し、ダブリング realloc で in-place に append するヘルパに置き換えているため、ループ全体が O(n) で済みます (他の言語の多くは O(n²) のまま)。実測は 5 万回 append で wall 約 3 ms。
