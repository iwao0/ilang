...
- **エラーの「層」を見分ける。** `mir-codegen: ... Verifier(... mismatched argument count for jump ...)` は **不正な IR を生成したバグ**(型エラーではない)。 `mir lower: no coercion from () to i64` のような lower 段のメッセージは、 checker が拒否すべきものを素通りさせている兆候(第 159 弾)。 どちらもソース位置付きの綺麗な checker 診断にすべき所で出ていたら穴。 ソース位置付き(`file [r:c]: ...`)= checker の正規診断、 ソース位置なしの `mir-codegen:`/`mir lower:` = 内部段まで漏れた = 多くは要修正、 と読み分ける(第 155〜159 弾はこの読み分けで 5 件を切り分けた)。
...
**継ぎ目が無い(直近がドキュメント変更等)ときは「最近 fixture 化されていない領域」を狙う。** ディレクトリ別の最終 fixture 追加日を出して最古を選ぶ:

```bash
for d in crates/ilang-cli/tests/programs/*/; do
  echo "$(git log -1 --format=%ai -- "$d" | cut -d' ' -f1)  $(basename "$d")"
done | sort
```

第 160 弾はこれで最古の `06_overloading`/`07_method_overloading`(他より 2 週間古い)を選び、 overload 選択 × heap ARC を pin した。

**狙う交点を決めたら、必ず [BUG_COVERAGE.md](BUG_COVERAGE.md) を grep してから probe を書く。** 同じ攻撃面の fixture が既にあれば確認済みなので別の交点へ移る — 同じ場所の再 probe を防ぐための索引である。HANDOFF を全部読まずに「まだ probe していない組合せ」を判定できるのが本表の目的。BUG_COVERAGE は全 fixture を各ヘッダコメントから逐語転記した自動生成カタログで、`scripts/gen_bug_coverage.sh` で再生成する。
...
- **発散 × 値 join 構文**: ある機能が新しい「発散する形」(`todo()`、 break 無し loop、 全 arm 発散 match 等)を足したら、 **値を join する全構文に同じ穴がある**と疑う。 値 join を持つのは match の **各種別**(enum / int / bool / string は別 lowering)・`if` 式・`loop` の break 値・そして**値消費位置**(引数・オペランド・配列/タプル/Map 要素)。 第 155〜159 弾は `todo()`/発散 break を起点に、 checker の break join → MIR の loop break → int/bool match → if 式 → 消費位置と**同型の穴を 5 連続**で潰した(enum/string match だけ先に対応済みで、 他が取り残されていた)。 「1 つの構文で発散を直したら、 残りの値 join 構文を全部 probe する」。
...
- fixture が落ちたら `target/fixture-failures.log` に詳細(expected / actual / stderr)が残る。**このログは追記式で過去ラウンドの記録が混在する。** 失敗内容を見る前に `rm -f target/fixture-failures.log` してから走らせる(古いエントリを「今の失敗」と誤読すると、 別の fixture を追いかけて時間を溶かす — 第 159 弾で実際に踏んだ)。
...
[ ] 狙う交点を BUG_COVERAGE.md で grep 確認した (既出なら別の交点へ) (§6)
