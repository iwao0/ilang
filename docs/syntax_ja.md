...
**新規のコンテナリテラル**はその要素型 / 値型に対して共変。`[new Dog()]` は `Animal[]` へ、`{"a": new Dog()}` は `Map<string, Animal>` へ流れる (`let` 注釈下では `{"a": new Dog(), "b": none}` → `Map<string, Animal?>` のような混在も可)。**generic enum のコンストラクタリテラル**も全型引数位置で同様に共変 — `Result.ok(new Dog())` は `Result<Animal, string>` へ、`Result.err(new Dog())` は `Result<i64, Animal>` へ流れる。これはリテラル限定 — 別名の `Map<string, Dog>` / `Result<Dog, string>` **変数**は親型版へ代入できない (親型越しの変更が unsound になるため。enum 値は immutable なのでリテラル形は健全)。各腕が新鮮なリテラル (コンストラクタリテラル・配列/マップ/タプルリテラル・それらを包む `some(..)`) である `if` / `match` の合流にも共変が及ぶ — `if c { Box.hold(new Dog()) } else { Box.hold(new Cat()) }` は `Box<Animal>` へ、`if c { [new Dog()] } else { [new Cat()] }` は `Animal[]` へ、`if c { (new Dog(), 1) } else { (new Cat(), 2) }` は `(Animal, i64)` へ合流する (素のオブジェクトの if 合流が共通祖先を取るのと同様)。リテラル限定は維持され、いずれかの腕が別名の generic / 配列 / マップ / タプル変数なら合流は不変のまま。
...
タプル・動的配列 (`T[]`)・オプショナル (`T?`) の `==`/`!=` は **構造的**。要素ごとに比較し、入れ子のタプル/配列/オプショナルへ再帰し、各スロットの種類に応じた値等価を用いる — 数値/真偽値は値で、文字列と enum は構造的に、オブジェクト (その他の参照型) は参照で比較する。よって `(1, "a") == (1, "a")` や `[1, 2] == [1, 2]` は `true`、別インスタンス `p`/`q` の `(p, 1) == (q, 1)` は `false`。オプショナルは `none` とも比較できる (`x == none`)。これらの順序比較 (`<` 等) は型エラーのまま。配列の `indexOf` / `includes` / `remove` の要素一致も同じ構造的等価を使う。
...
すべての数値プリミティブ (`i8`〜`u64` / `f32` / `f64`) と `bool` で利用可。浮動小数のフォーマットは `console.log` と同じ。`u64` の値 ≥ 2^63 は符号なし 10 進で表示される (`18000000000000000000`、 負の 2 の補数読みでない) — `toString` / `console.log` / 配列の表示 / `${}` 補間すべてで同じ。

裸の整数リテラルは相手のオペランドの整数型を採用するので、 `u64_val / 2` ・ `% 2` ・ `>> 2` は符号なしの除算 / 剰余 / 論理シフトを使う (最上位ビットが立った `u64` への符号付き演算は誤りになる)。 **変数どうし**の真の cross-sign 演算は従来どおり「広い型へ昇格・同幅なら符号付き優先」。
...
- **分岐の合流 (branch join)**: `if` / `match` の枝、配列リテラル `[..]`、Map リテラルの値が**異なるクラス**を返すとき、共通の親クラスがあればその親型に合流する。親クラスを共有しなくても**唯一の共通インタフェースを実装している**ならそのインタフェース型に合流する (`if c { new Circle() } else { new Square() }` は両者が `Shape` を実装していれば `Shape`)。共通インタフェースが**複数**あるときは合流先が曖昧になり型エラー — 注釈を付けるか構造を変える。
...
`keys()` / `values()` / `entries()` / `forEach`・印字はすべて
**挿入順**で反復します (JS の `Map` と同じ): キーの上書きは元の
位置を保ち、`delete` して再挿入すると末尾に移ります。`Set` の反復と
集合演算も同様です (`union` は 1 つ目の集合 → 2 つ目の順)。
...
- repr enum 同士は discriminant で比較する。等価 (`==` / `!=`) と順序 (`<` `<=` `>` `>=`) の両方が使え、enum 同士でも repr 型の整数リテラルとでも比較できる (`Priority.low < Priority.high`, `Priority.high == 10`)。符号付き repr (`: i32`) は符号付きの値で順序付けされる。`:` repr を持たない素の enum は `==` / `!=` (バリアントタグ上) のみで、順序比較は型エラー
