...
| 配列 | `[1, 2, 3]`, `[1, 2, 3,]` (末尾コンマ可) | `T[N]` (固定長) — 動的配列にしたい場合は `T[]` を注釈 |
...
let zs: i32[] = []              // 空配列は注釈必須
let inferred = [1, 2, 3]        // 注釈なしは固定長 `i64[3]` に推論
let trailing = [1, 2, 3,]       // 末尾コンマ可 (こちらも固定長)
...
注釈のない配列リテラルは **固定長** に推論されます。`let a = [1,
2, 3]` は `i64[]` ではなく `i64[3]`。後述の破壊的操作 (`push` /
`pop` / `shift` / `unshift` / `remove` / `removeAt` / `fill`) は
...
`arr[i] = x` は可)、generic の型パラメータに解決させる
(`fn hold<T>(v: T)` への引数渡しでの推論・`new C<Box[2]>` の明示
どちらも不可 — 引数型を `Box[2]` と直接書けば可)。
