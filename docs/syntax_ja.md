...
field・束縛・引数に加えて、**Optional のセルと enum payload** にも
入れられます。field 代入と同じく、格納時に値コピーが作られます:
...
引き続き型エラーになるもの: 戻り値型に使う、動的配列 / Map / Set /
tuple の構成要素にする、closure にキャプチャする、束縛全体を再代入
する (`arr = [..]` — 要素の上書き `arr[i] = x` は可)。
