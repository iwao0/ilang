...
- 整数範囲パターン (`a..b`, `a..=b`) は `a <= x < b` / `a <= x <= b` で一致。両端は整数リテラル (または `-Lit`) のみ。空レンジ (`5..5`, `5..3`) はコンパイルエラー
...
| `.parent` | `Type?` | `extends` で指定された直近の親クラス。非クラスやルートクラスは `none` |
...
| `.typeArgs` | `Type[]` | ジェネリックインスタンスの型引数 (`Result<i64, string>` なら `[Type("i64"), Type("string")]`)。非ジェネリックは空。**JIT のみ動作** — interpreter は現状ジェネリックでも空配列を返す |
...
- `.fields` / `.methods` は **宣言された** メンバ名のみを返し、継承や型情報は含まない (型情報は今後の拡張予定)
- `.typeArgs` は現状 JIT のみ。interpreter は値ごとの型引数を保持しないため、`Result<i64, string>` でも空配列を返します
...
@extern(C) {
    ...
    // C グローバル変数
    static stdout_fd: i32   // host 登録 (例)
}
```
...
#### 静的グローバル変数 (`static name: T`)

```rust
@extern(C) {
    @lib("c") static optarg: *const char    // dlsym で解決
    static test_static_i32: i32             // host 登録形
}
```

- 型は **数値プリミティブ / bool** のみ
- read/write は load/store として直接 address に対して行われる
- 用途: `errno`(別途 `os.errno()` あり) / `optarg` / `tzname` 等の C global
...
- 同じ `@extern(C) { ... }` ブロック内で複数の fn / struct / static を宣言する場合、宣言順序は自由
